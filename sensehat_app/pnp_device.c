...
#define SENSEHAT_INDEX 0

#define DEVICEINFO_INDEX 1

#define DEFAULT_SEND_TELEMETRY_INTERVAL_MS 10000

#define DEVICE_CAPABILITY_MODEL_URI "urn:sense_hat:SenseHatModel:1"
...
int pnp_device_initialize(const char* connectionString, const char* trustedCert)
{
    ...
    if ((interfaceClientHandles[SENSEHAT_INDEX] = SensehatInterface_Create(digitalTwinDeviceClientHandle)) == NULL)
    {
        LogError("SensehatInterface_Create failed");
        return -1;
    }
...
void pnp_device_run()
{
    ...
    if (nowTick - lastTickSend >= DEFAULT_SEND_TELEMETRY_INTERVAL_MS)
    {
        ...
        SensehatInterface_Telemetry_SendAll();
...
{
    if (interfaceClientHandles[SENSEHAT_INDEX] != NULL)
    {
        SensehatInterface_Close(interfaceClientHandles[SENSEHAT_INDEX]);
    }
