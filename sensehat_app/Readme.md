...
1. Copy the folder of **sensehat_app** with the generated code into the source folder **azure-iot-sdk-c-pnp** .

1. Open the `CMakeLists.txt` in the **azure-iot-sdk-c-pnp** folder. Include the **sensehat_app** folder so that it will be built together with the Device SDK. Add the line below to the end of the file.
    ```txt
    add_subdirectory(sensehat_app)
    ```
...
1. Once the build has succeeded, you can test it by specifying the IoT Hub device connection string as its parameter.
    ```bash
    \\sensehat_app\\Release\\sensehat_app.exe "[IoTHub device connection string]"
    ```
...
1. Once the build has succeeded, you can test it by specifying the IoT Hub device connection string as its parameter.
    ```bash
    ./sensehat_app/sensehat_app "[IoTHub device connection string]"
    ```